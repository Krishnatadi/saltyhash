...
class SaltyHash {
   ...
   */
  static async hash(input, rounds = 12, algorithm = 'sha256', version = '2a') {
    input = String(input);
    // Validate rounds
    if (!input || input.trim() === '') {
    ...
    // Generate a random salt using pbkdf2 (asynchronous version)
    const salt = await new Promise((resolve, reject) => {
      crypto.pbkdf2(input, crypto.randomBytes(16), rounds, 22, algorithm, (err, derivedKey) => {
        if (err) reject(err);
        resolve(derivedKey.toString('base64').slice(0, 22));
      });
    });
   ...
   */
  static async generateSaltFromKDF(input, rounds = 100000, algorithm = 'sha256') {
    // Validate inputs
    if (!input || input.trim() === '') {
      throw new Error('Input password is required and cannot be empty.');
    }
    ...
    // Generate the salt using pbkdf2 (asynchronous version)
    const salt = await new Promise((resolve, reject) => {
      crypto.pbkdf2(input, crypto.randomBytes(16), rounds, 22, algorithm, (err, derivedKey) => {
        if (err) reject(err);
        resolve(derivedKey.toString('base64').slice(0, 22));
